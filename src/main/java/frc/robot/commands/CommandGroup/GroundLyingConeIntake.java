...
public class GroundLyingConeIntake extends SequentialCommandGroup {
  /** Creates a new GroundLyingConeIntake. */
  public GroundLyingConeIntake(PivotArmSubsystem pivotArmSubsystem,
  ExtensionArmSubsystem extensionArmSubsystem,
  WristSubsystem wristSubsystem,
  IntakeSubsystem intakeSubsystem) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(new ParallelDeadlineGroup(new PivotAtTargetPosition(pivotArmSubsystem, -15, false), new TogglePivotTargetPosition(pivotArmSubsystem, -15))
    ,new ParallelDeadlineGroup(new ExtensionAtTargetPosition(extensionArmSubsystem, -19, false), new ToggleExtensionTargetPosition(extensionArmSubsystem, -19))
    ,new ToggleWristTargetPosition(wristSubsystem, -102));
  }
}
