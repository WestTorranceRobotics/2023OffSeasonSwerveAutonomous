...
public class ScoreHighCone extends SequentialCommandGroup {
  ...
  public ScoreHighCone(PivotArmSubsystem pivotArmSubsystem,
  ExtensionArmSubsystem extensionArmSubsystem,
  WristSubsystem wristSubsystem,
  IntakeSubsystem intakeSubsystem) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(new ParallelDeadlineGroup(new PivotAtTargetPosition(pivotArmSubsystem, -152, false), new TogglePivotTargetPosition(pivotArmSubsystem, -152)),new Delay(0.75)
    ,new ParallelDeadlineGroup(new WristAtTarget(wristSubsystem, -84, false),new ToggleWristTargetPosition(wristSubsystem, -84)
    ,new ParallelDeadlineGroup(new ExtensionAtTargetPosition(extensionArmSubsystem, -37, false), new ToggleExtensionTargetPosition(extensionArmSubsystem, -37))
    )
    );
  }
}
